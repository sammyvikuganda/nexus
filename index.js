...
app.get('/dashboard', async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            investmentData = await investmentResponse.json();

            // Fallback: if no investment or amount is 0
            if (!investmentData || !investmentData.amount || investmentData.amount === 0) {
                investmentData = {
                    amount: 0,
                    payout: 0,
                    premium: 0,
        ...
        if (investmentData.premium > 0) {
            ...
            dailyPayout = (investmentData.amount * 0) / 100; // fallback 0% if no plan
        }
        ...
        res.send(`
            ...
            <body>
                <div id="content">
                            ...
                            <div class="scrollable-content">
                                ...
                                <div class="nexusweb-quick-access">
                                    ...
                                    <div class="nexusweb-quick-access-item" id="nexusweb-wallet-item">
                                        <i class="fas fa-wallet"></i> USD Wallet
                                    </div>
                                </div>
                    ...
                    <div id="investments-section" class="section" style="display: block;">
                        <div class="content">
                ...
                <script>
                    // Countdown logic
                    let remainingTime = ${remainingTime}; // Set the remaining time from backend

                    function countdown() {
                        if (remainingTime <= 0) {
                            document.getElementById('countdownTimer').innerText = "Time's up!";
                        } else {
                            let hours = Math.floor(remainingTime / (1000 * 60 * 60));
                            let minutes = Math.floor((remainingTime % (1000 * 60 * 60)) / (1000 * 60));
                            let seconds = Math.floor((remainingTime % (1000 * 60)) / 1000);
                            document.getElementById('countdownTimer').innerText = `${hours}h ${minutes}m ${seconds}s remaining`;
